...
impl VulkanVersion {
    /// Create a new [`VulkanVersion`]
    pub const fn new(variant: u8, major: u8, minor: u16, patch: u16) -> VulkanVersion {
        ...
        unsafe { VulkanVersion::new_raw(version) }
    }

    /// Create a new [`VulkanVersion`] from a raw `version` value
    pub(crate) const unsafe fn new_raw(version: u32) -> VulkanVersion {
