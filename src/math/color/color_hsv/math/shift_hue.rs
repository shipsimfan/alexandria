...
impl<T, Space: ColorSpace<T>> ColorHsv<T, Space> {
    ...
    pub const fn shift_hue(self, amount: T) -> ColorHsv<T, Space>
    where
    ...
    ///
    /// The saturation and value channels are unchanged.
    pub const fn add_hue(mut self, amount: T) -> ColorHsv<T, Space>
    where
        T: [const] NormalizedWrappingAdd + [const] Destruct,
    {
    ...
    ///
    /// The saturation and value channels are unchanged.
    pub const fn sub_hue(mut self, amount: T) -> ColorHsv<T, Space>
    where
