...
/// The converter for [`ColorHsva`]
struct ColorHsvaConverter<'de, T: Deserialize<'de>, Space: ColorSpace<T>> {
...
impl<'de, T: Deserialize<'de>, Space: ColorSpace<T>> Deserialize<'de> for ColorHsva<T, Space> {
    fn deserialize<D: data_format::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        deserializer.deserialize_map(ColorHsvaConverter::default())
    }
}

impl<'de, T: Deserialize<'de>, Space: ColorSpace<T>> Default for ColorHsvaConverter<'de, T, Space> {
    fn default() -> Self {
        ColorHsvaConverter {
            _type: PhantomData,
            _space: PhantomData,
...
impl<'de, T: Deserialize<'de>, Space: ColorSpace<T>> Converter<'de>
    for ColorHsvaConverter<'de, T, Space>
{
    type Value = ColorHsva<T, Space>;

    fn expecting(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "a HSVA color")
    }
    ...
    fn convert_map<M: data_format::MapDeserializer<'de>>(
        ...
        while let Some(key) = map.next_key::<Cow<'de, str>>()? {
            match key.as_ref() {
                ...
                _ => return Err(M::Error::unknown_field(key, &["h", "s", "v", "a", "space"])),
            }
        }
