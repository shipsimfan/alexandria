...
impl<T> Matrix4x4<T> {
    ...
    pub const fn from_rows([r0, r1, r2, r3]: [Vector4<T>; 4]) -> Matrix4x4<T>
    where
        T: [const] Destruct,
    {
    ...
    pub const fn from_cols([c0, c1, c2, c3]: [Vector4<T>; 4]) -> Matrix4x4<T>
    where
        T: [const] Destruct,
    {
...
const impl<T: [const] Destruct> From<[Vector4<T>; 4]> for Matrix4x4<T> {
