use crate::math::{Vector3, number::Clamp};
use std::marker::Destruct;

impl<T> Vector3<T> {
    /// Clamp this vector component-wise between `min` and `max`
    pub const fn clamp_v(self, min: Vector3<T::Bound>, max: Vector3<T::Bound>) -> Self
    where
    ...
    {
        self.clamp_v(Vector3::splat(min), Vector3::splat(max))
    }
}

impl<T: [const] Clamp + [const] Destruct> const Clamp for Vector3<T>
where
    T::Bound: [const] Clone + [const] Destruct,
{
    type Bound = T::Bound;
